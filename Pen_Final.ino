...
void handle_start(){
  ...
  automode=true;
  pen_status="Moving in Auto Mode";
  server.send(200, "text/html", SendHTML());
  autostart();
  pen_status="Cycle Finish";
  server.send(200, "text/html", SendHTML());
  
}
void handle_manual(){
  Serial.println("Going Manual");
  pen_status="Going Manual";
  server.send(200, "text/html", SendHTML());
...
void handle_gohome(){
  Serial.println("Go Home");
   pen_status="Going Home";
  server.send(200, "text/html", SendHTML());
  myMotorDriver.setDrive( LEFT_MOTOR, 0, 127); // Going Left
  delay(2);
  ...
  halt();
  myMotorDriver.setDrive( LEFT_MOTOR, 0, 130); // Going REV
   delay(1);
...
void handle_fwd(){
   pen_status="Going FWD";
  server.send(200, "text/html", SendHTML());
  if(automode==false){
    fwd(len);}
}
void handle_rev(){
   pen_status="Going REV";
...
void handle_right(){
   pen_status="Going RIGHT";
  server.send(200, "text/html", SendHTML());
  if(automode==false){
    right(len);}
}
void handle_left(){
   pen_status="Going LEFT";
  server.send(200, "text/html", SendHTML());
  if(automode==false){
   left(len);}
}
...
void autostart(){
  Serial.println("Start in Auto Mode");
      ...
      if(!autofaultpresent()){
        pause(int(500));}
      if(!autofaultpresent()){
        rightfwd(328);} // Diagonal Cap
     if(!autofaultpresent()){
        pause(int(500));}
     if(!autofaultpresent()){
       rightrev(328);} // Diagonal Cap
     if(!autofaultpresent()){
       pause(int(500));}
     if(!autofaultpresent()){
       leftrev(658);}  //Diagonal Big
      if(!autofaultpresent()){
       pause((int(500)));}
      if(!autofaultpresent()){ 
       fwd(300);} // West Side
      if(!autofaultpresent()){
       pause((int(500)));}
     if(!autofaultpresent()){
        rightrev(694);} // Diagonal Big
      if(!autofaultpresent()){
        pause((int(500)));}
//      if(!autofaultpresent()){
//        left((int(1609/3)));}// Back to Start
      if(!autofaultpresent()){
